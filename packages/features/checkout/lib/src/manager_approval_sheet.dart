...
/// Ask a manager (PERMISSIONS_ARCHITECTURE §4.2): a manager types THEIR PIN
/// on this device, over the teller's session. [approve] hands the PIN to the
/// core, which finds them, checks they may approve this act for this person,
/// and returns the approval the act then carries. Pops the approval, or null
/// when dismissed. Shared by every act that can need a manager (void, refund,
/// discount).
Future<ApprovalView?> askManagerWith(
  BuildContext context, {
  required String reason,
  required Future<ApprovalView> Function(String pin) approve,
}) {
  return showMadarSheet<ApprovalView>(
    context,
    size: SheetSize.hug,
    maxWidth: 420,
    builder: (_) => _ApprovalSheet(reason: reason, approve: approve),
  );
}

class _ApprovalSheet extends ConsumerStatefulWidget {
  const _ApprovalSheet({required this.reason, required this.approve});
...
class _ApprovalSheetState extends ConsumerState<_ApprovalSheet> {
  ...
  Future<void> _approve() async {
    ...
    try {
      final approval = await widget.approve(_pin.text.trim());
