...
impl Fixture {
    ...
    async fn cleanup(&self) {
        ...
        let _ = self
            .db
            .execute("DELETE FROM orders WHERE branch_id = $1", &[b])
            .await;
        let _ = self
            .db
            .execute("DELETE FROM shifts WHERE branch_id = $1", &[b])
            .await;
        let _ = self
            .db
            .execute("DELETE FROM branches WHERE id = $1", &[b])
            .await;
        let _ = self
            .db
            .execute(
...
async fn provision_fixture() -> Fixture {
    ...
    let db_url = env(
        "MADAR_IT_DB",
        "postgres://madar:madar@localhost:5432/madar_dev",
    );
    let (db, conn) = tokio_postgres::connect(&db_url, tokio_postgres::NoTls)
        .await
        .expect("dev DB connect");
...
async fn offline_login_unlocks_after_an_online_login() {
    ...
    match &r {
        Ok(s) => eprintln!(
            "OFFLINE-LOGIN OK: user_id={} name={} org={:?}",
            s.user_id, s.display_name, s.org_id
        ),
    ...
    match &r2 {
        Ok(s) => eprintln!(
            "SIGN_IN(offline) OK: user_id={} name={}",
            s.user_id, s.display_name
        ),
...
async fn captive_portal_login_falls_back_to_offline() {
    ...
    assert!(
        !session.online,
        "a portal-fallback session is offline, not online"
    );
...
async fn offline_backlog_replays_exactly_once_across_a_reconnect() {
    ...
    for _ in 0..2 {
        ...
        assert!(
            r.queued_offline,
            "an offline sale must queue, not report sent"
        );
    }
    let _ = offline
...
async fn full_offline_day_open_sell_close_replays_in_dependency_order() {
    ...
    if let Ok(Some(s)) = live.refresh_till().await {
        if s.is_open {
            live.close_till(0, None).await.ok();
            live.sync_now().await.ok();
        }
    }
    let blob = session_blob(&db_path);
...
async fn another_teller_flushes_the_backlog_attributed_to_the_original() {
    ...
    let b_session = b
        .login(login(&teller_b))
        .await
        .expect("B must be able to sign in despite A's open shift");
    ...
    assert_eq!(
        s2.teller_id, teller_a_id,
        "S2 attributed to A — the teller who opened it"
    );
...
async fn offline_teller_switch_close_open_syncs_on_reconnect() {
    ...
    assert_ne!(
        shift.teller_id, t1_id,
        "teller 1's S1 must be CLOSED server-side"
    );

    // Cleanup: close S2 as t2 so the fixture is tidy.
    t2core.close_till(0, None).await.ok();
    t2core.sync_now().await.ok();
    fx.cleanup().await;
    let _ = std::fs::remove_file(&db);
}
...
async fn offline_orphaned_orders_recover_onto_the_real_shift() {
    ...
    let otherdb = std::env::temp_dir().join(format!(
        "madar_it_orphan_other_{}.sqlite",
        std::process::id()
    ));
    ...
    if let Ok(Some(s)) = other.refresh_till().await {
        if s.is_open {
            other.close_till(0, None).await.ok();
    ...
    let maindb = std::env::temp_dir().join(format!(
        "madar_it_orphan_main_{}.sqlite",
        std::process::id()
    ));
    ...
    assert!(
        !main_online.list_menu_items().unwrap().is_empty(),
        "catalog present on main"
    );
    ...
    let cur = main
        ...
        .expect("active shift");
    assert!(
        cur.is_open && cur.id == a.id,
        "main routes into the REAL shift A ({} vs {})",
        cur.id,
        a.id
    );

    // Cleanup: close A so the fixture branch is tidy.
    main.close_till(0, None).await.ok();
    main.sync_now().await.ok();
    fx.cleanup().await;
    let _ = std::fs::remove_file(&maindb);
    let _ = std::fs::remove_file(&otherdb);
...
async fn offline_receipt_number_and_ref_match_the_server() {
    ...
    let predicted = off
        ...
        .expect("offline order");
    eprintln!(
        "OFFLINE PREDICTED: #{:?} ref={:?}",
        predicted.order_number, predicted.order_ref
    );
    assert!(
        predicted.order_ref.is_some(),
        "offline receipt must carry a predicted ref"
    );
    ...
    let minted = server_orders
        .iter()
        .find(|o| o.order_ref == predicted.order_ref);
    eprintln!(
        "SERVER REFS: {:?}",
        server_orders
            .iter()
            .map(|o| (o.order_number, o.order_ref.clone()))
            .collect::<Vec<_>>()
    );
    let minted = minted.unwrap_or_else(|| {
        panic!(
    ...
    assert_eq!(
        minted.order_number.map(|n| n as i64),
        predicted.order_number,
        "order_number must match the server's"
    );

    back.close_till(0, None).await.ok();
    back.sync_now().await.ok();
    fx.cleanup().await;
    let _ = std::fs::remove_file(&db);
}
...
async fn login_rejected_taking_over_another_tellers_open_shift() {
    ...
    let t2db = std::env::temp_dir().join(format!(
        "madar_it_takeover_t2_{}.sqlite",
        std::process::id()
    ));
    ...
    assert!(
        r.is_err(),
        "teller 2 must NOT sign in over teller 1's open shift (no handover ack)"
    );
...
async fn cash_movement_is_offline_first_and_idempotent() {
    ...
    assert!(
        after
            .iter()
            .any(|m| m.note == "rebuild test pay-in" && m.amount_minor == 2_500),
    ...
    assert_eq!(
        after.len(),
        before + 1,
        "client_ref must dedup — no duplicate movement"
    );
    fx.cleanup().await;
}
...
async fn offline_lists_show_last_synced_server_rows() {
    ...
    assert!(
        on_cash.iter().any(|m| m.note == "online float"),
    ...
    assert!(
        !off_shifts.is_empty(),
        "past shifts must still show offline (from cache)"
    );
...
async fn offline_closed_shift_shows_closed_not_active_in_past_shifts() {
    ...
    let off = core_at("http://127.0.0.1:1".into(), db_path.clone());
    off.restore_session(blob.clone());
    off.close_till(11_000, None).await.expect("close offline");
    ...
    assert!(
        !row.is_open,
        "a shift closed offline must show CLOSED, not active, in past shifts"
    );
    assert_eq!(row.status, "closed", "status overlaid to closed");
    assert_eq!(
        row.closing_declared_minor,
        Some(11_000),
        "shows the locally-declared closing cash"
    );
...
async fn offline_opened_and_closed_shift_is_complete_in_history() {
    ...
    off.close_till(9_500, None).await.expect("close B offline");
    ...
    assert_eq!(
        row.closing_declared_minor,
        Some(9_500),
        "carries the declared closing cash"
    );
    ...
    assert_eq!(
        orders.len(),
        1,
        "the offline shift's order shows in its history"
    );
    ...
    assert!(
        !report.from_server,
        "reconstructed locally, not from the server"
    );
...
async fn fresh_shift(core: &MadarCore, tag: &str) -> String {
    if let Ok(Some(s)) = core.refresh_till().await {
        if s.is_open {
            core.close_till(0, None).await.ok();
            core.sync_now().await.ok();
        }
    }
...
async fn online_order_numbers_match_the_server_and_increment() {
    ...
    for _ in 0..3 {
        ...
        assert!(
            !r.queued_offline,
            "an online sale syncs immediately, not queued"
        );
        receipts.push(r);
    }
    ...
    assert_eq!(
        ...
        "online numbers must increment, not stick at #1"
    );
...
async fn ringup_receipt_matches_the_server_reprint() {
    ...
    core.sync_now().await.ok();
    assert!(
        ringup.order_number.is_some() && ringup.order_ref.is_some(),
        "ring-up carries both"
    );
    ...
    let row = server
        .iter()
        .find(|o| o.order_ref == ringup.order_ref)
        .expect("order on server");
    let reprint = core
        .order_receipt_view(row.id.clone())
        .await
        .expect("reprint");

    assert_eq!(
        reprint.order_number, ringup.order_number,
        "reprint #N == ring-up #N"
    );
    assert_eq!(
        reprint.order_ref, ringup.order_ref,
...
async fn resuming_a_shift_mid_day_predicts_max_plus_one() {
    ...
    assert_eq!(
        r3.order_number,
        Some(3),
        "resumed shift continues at MAX+1 (#3), not #1"
    );
    ...
    let matched = server
        .iter()
        .find(|o| o.order_ref == r3.order_ref)
        .expect("ref on server");
    assert_eq!(
        matched.order_number.map(|n| n as i64),
        Some(3),
        "server stored #3 too"
...
async fn online_then_offline_order_numbers_stay_contiguous() {
    ...
    assert_eq!(
        (o3.order_number, o4.order_number),
        (Some(3), Some(4)),
        "offline continues the sequence"
    );
    assert!(
        o3.queued_offline && o4.queued_offline,
    ...
    let mut server_nums: Vec<i64> = server
        .iter()
        .filter_map(|o| o.order_number.map(|n| n as i64))
        .collect();
    server_nums.sort_unstable();
    assert_eq!(
        server_nums,
        vec![1, 2, 3, 4],
        "server holds a contiguous 1..4 with no gaps or dups"
    );
    for r in [&o3, &o4] {
        let m = server
            .iter()
            .find(|o| o.order_ref == r.order_ref)
            .expect("offline ref on server");
        assert_eq!(
            m.order_number.map(|n| n as i64),
            r.order_number,
            "offline receipt == server"
        );
    }
