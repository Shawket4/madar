...
pub struct CheckoutCommand {
    ...
    /// Who started the order when it is not the person ringing it: a held
    /// order resumed after a teller switch (queue.rs rule 5). Absent on every
...
/// `"36B-12"` — the number people read (contract §4.7). madar-shared's
/// `madar_ids::order_ref::display_number`.
pub fn display_number(device_code: &str, order_number: i64) -> String {
    madar_ids::order_ref::display_number(device_code, order_number)
}
...
pub fn display_number_from_ref(order_ref: Option<&str>, order_number: i64) -> String {
    madar_ids::order_ref::display_number_from_ref(order_ref, order_number)
}
...
pub(crate) fn order_envelope(
    ...
    if let (Some(by), Some(obj)) = (&cmd.started_by, request.as_object_mut()) {
        obj.insert("started_by".into(), by.clone().into());
    }
...
pub(crate) fn mint_order_ref(
    ...
    Some((
        ref_seq,
        madar_ids::order_ref::device_ref(&branch_code, &yymmdd, &device, ref_seq),
    ))
}
...
pub(crate) fn prepare(
    ...
    let raw =
        raw_payment_method(store, payment_method_id)?.ok_or_else(|| CoreError::Validation {
    ...
    let payment_label =
        display_label(store, locale, payment_method_id).unwrap_or_else(|| raw.name.clone());
    ...
    let tip_is_cash = match &input.tip_payment_method_id {
        Some(id) => raw_payment_method(store, id)?
            .map(|p| p.is_cash)
            .unwrap_or(is_cash),
        None => is_cash,
    };
    ...
    if !input.splits.is_empty() {
        let legs: Vec<models::PaymentSplitInput> = input
            ...
            .iter()
            .filter_map(|s| {
                let name = raw_payment_method(store, &s.payment_method_id)
                    .ok()
                    .flatten()?
                    .name;
                Some(models::PaymentSplitInput {
...
fn order_cash_in_drawer(
    ...
    let sale_cash: i64 = match req.payment_splits.as_ref().and_then(|s| s.as_ref()) {
        Some(legs) if !legs.is_empty() => legs
            ...
            .sum(),
        _ if cash_names.contains(&req.payment_method) => {
            req.total_amount.flatten().unwrap_or(0) as i64
        }
        _ => 0,
    };
    ...
    let tip_cash = if tip > 0 {
        ...
        if cash_names.contains(tip_method) {
            tip
        } else {
            0
        }
...
mod tests {
    ...
    fn cash_order_assembles_priced_with_change() {
        ...
        assert_eq!(
            r.items[0].menu_item_id,
            Some(Some(uuid::Uuid::parse_str(ITEM).unwrap()))
        );
    ...
    fn queued_cash_total_sums_only_cash_orders() {
        ...
        let queue = |id: &str, method: &str, total: i32| {
            ...
            let cmd = CheckoutCommand { request: req, device: None, started_by: None, approval: None };
            store
    ...
    fn queued_cash_total_counts_only_cash_legs_and_cash_tips() {
        ...
        push("split", split);
        assert_eq!(
            queued_cash_total(&store).unwrap(),
            600,
            "split counts only the cash leg, not the whole total"
        );
    ...
    fn order_base_defaults_zero_is_monotonic_and_per_shift() {
        ...
        assert_eq!(
            order_base(&store, SHIFT),
            5,
            "monotonic — a lower number is ignored"
        );
        bump_order_base(&store, SHIFT, 9);
        assert_eq!(order_base(&store, SHIFT), 9);
        const OTHER: &str = "00000000-0000-0000-0000-0000000000c9";
        assert_eq!(
            order_base(&store, OTHER),
            0,
            "per-shift isolation — OTHER keeps its own base"
        );
    }
    ...
    fn ref_seq_is_monotonic_per_day_and_independent_of_order_number() {
        ...
        assert!(
            r2.ends_with("-0002"),
            "second ref = 0002 regardless of order_number: {r2}"
        );
        let (_n3, r3) = mint_order_ref(&store, SHIFT, "2026-06-21T09:00:00+00:00").expect("mint3");
        assert!(
            r3.ends_with("-0001"),
            "ref sequence resets per business day: {r3}"
        );
    }
    ...
    fn card_tip_does_not_reduce_cash_change() {
        ...
        assert_eq!(
            p.command.request.tip_payment_method,
            Some(Some("Card".into()))
        );
    }
    ...
    fn percentage_discount_is_recorded_on_the_wire_and_receipt() {
        ...
        assert_eq!(
            r.discount_id,
            Some(Some(
                uuid::Uuid::parse_str("00000000-0000-0000-0000-0000000000d1").unwrap()
            ))
        );
    ...
    fn queued_cash_total_adds_signed_cash_movements() {
        ...
        store
            .enqueue(&crate::store::NewOutboxOp {
                ...
                payload: serde_json::to_string(&CheckoutCommand { request: req, device: None, started_by: None, approval: None }).unwrap(),
                event_at: "2026-06-20T12:00:00+00:00".into(),
                ..Default::default()
            })
            .unwrap();
    ...
    fn queued_cash_total_for_is_scoped_to_one_shift() {
        ...
        assert_eq!(
            queued_cash_total_for(&store, A).unwrap(),
            2300,
            "A: cash 2000 + movement 300"
        );
        assert_eq!(
            queued_cash_total_for(&store, B).unwrap(),
            5100,
            "B: cash 5000 + its own movement 100"
        );
    }
    ...
    fn queued_cash_total_excludes_inflight_orders() {
        ...
        assert_eq!(
            queued_cash_total(&store).unwrap(),
            0,
            "inflight excluded (the synced report has it)"
        );
    }
    ...
    fn normal_wire_item_carries_size_and_unit_price() {
        ...
        assert_eq!(
            item.menu_item_id,
            Some(Some(uuid::Uuid::parse_str(ITEM_UUID).unwrap()))
        );
    ...
    fn receipt_filters_blank_customer_name() {
        ...
        .unwrap();
        assert_eq!(
            p.receipt.customer_name, None,
            "blank name must not reach the receipt"
        );
