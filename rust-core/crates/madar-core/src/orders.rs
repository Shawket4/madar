...
fn loc(translations: &serde_json::Value, base: &str, locale: &str) -> String {
    crate::menu::resolve(translations, base, locale)
}

/// Qty-suffix an addon label ("Oat milk ×2") after localizing its name.
fn addon_label(name: &str, translations: &serde_json::Value, qty: i32, locale: &str) -> String {
    let n = loc(translations, name, locale);
    if qty > 1 {
        format!("{n} ×{qty}")
    } else {
        n
...
pub(crate) fn order_to_receipt(
    ...
    crate::checkout::ReceiptView {
        ...
        delivery_zone: dinfo
            .as_ref()
            .and_then(|d| d.zone_name.clone().flatten())
            .filter(|s| !s.is_empty()),
        delivery_ref: dinfo
            .as_ref()
            .and_then(|d| d.delivery_ref.clone().flatten())
            .filter(|s| !s.is_empty()),
        payment_hint: dinfo
            .as_ref()
            .and_then(|d| d.payment_method_hint.clone().flatten())
            .filter(|s| !s.is_empty()),
        delivery_notes: dinfo
            .as_ref()
            .and_then(|d| d.delivery_notes.clone().flatten())
            .filter(|s| !s.is_empty()),
...
fn compose_address(d: &models::OrderDeliveryInfo, locale: &str) -> Option<String> {
        ...
        .filter(|s| !s.trim().is_empty())
    {
        parts.push(format!(
            "{} {}",
            crate::i18n::tr(locale, "delivery.unit"),
            u
        ));
    }
    if let Some(f) = d.floor.clone().flatten().filter(|s| !s.trim().is_empty()) {
        parts.push(format!(
            "{} {}",
            crate::i18n::tr(locale, "delivery.floor"),
            f
        ));
    }
...
pub fn merge_for_view(
    ...
    let synced: std::collections::HashSet<&str> = server
        .iter()
        .filter_map(|o| o.order_ref.as_deref())
        .collect();
...
mod tests {
    ...
    fn summary_ref(total: i64, status: &str, order_ref: Option<&str>) -> OrderSummaryView {
        OrderSummaryView {
            order_ref: order_ref.map(|s| s.into()),
    ...
    fn merge_for_view_keeps_offline_only_and_refless_orders() {
        ...
        assert_eq!(
            merge_for_view(queued, server).len(),
            3,
