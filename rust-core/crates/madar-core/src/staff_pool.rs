...
//! ## The two copies
//!
//! The till decides offline, so this logic necessarily exists twice: here and
//! in `MadarRust/src/staff_pool/engine.rs`. They are pinned together by
//! `staff_pool_vectors.json`, a fixture committed to BOTH repos and executed by
//! a test on each side. Change the rule here and the backend test fails until
//! it is changed there too — which is the point. Do not fork it.
...
impl StaffDrinkRefusal {
    /// The i18n key of the teller-facing reason.
...
/// date, so the naive local date is exactly right here.
pub fn business_date_of(tz: chrono_tz::Tz, at: chrono::DateTime<chrono::Utc>) -> String {
    use chrono::TimeZone as _;
    tz.from_utc_datetime(&at.naive_utc()).date_naive().to_string()
}
...
mod tests {
    ...
    fn a_drink_inside_the_allowance_is_plain_allowed() {
        let d = decide(&settings(true, 5, &["latte"]), D, "latte", "for Sara, closing shift", 2);
    ...
    fn the_drink_past_the_allowance_lands_and_is_marked() {
        ...
        assert!(d.allowed, "an overspend is never refused — the drink was already made");
        assert!(d.overspent);
        assert_eq!(d.pool.remaining, 0);
    ...
    fn the_business_day_turns_over_at_branch_midnight_not_utc() {
        ...
        assert_eq!(business_date_of(cairo, late), "2026-09-20");
        ...
        assert_eq!(business_date_of(cairo, midnight), "2026-09-20");
        ...
        assert_eq!(business_date_of(cairo, before), "2026-09-19");
    }
