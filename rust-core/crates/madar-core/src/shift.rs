...
mod tests {
    ...
    fn reconcile_adopts_server_open_shift() {
        ...
        assert!(matches!(
            reconcile(&pf, TELLER_A, false, false),
            ShiftReconcile::Adopt(_)
        ));
        assert!(matches!(
            reconcile(&pf, TELLER_A, true, false),
            ShiftReconcile::Adopt(_)
        ));
    }
    ...
    fn reconcile_keeps_local_when_branch_shift_is_another_tellers_and_we_have_our_own() {
        ...
        assert!(matches!(
            reconcile(&pf, TELLER_B, true, false),
            ShiftReconcile::KeepLocal
        ));
    }
    ...
    fn reconcile_adopts_another_tellers_shift_only_when_we_have_no_local_open() {
        ...
        pf.open_shift = Some(Some(Box::new(open_shift_by(TELLER_A))));
        assert!(matches!(
            reconcile(&pf, TELLER_B, false, false),
            ShiftReconcile::Adopt(_)
        ));
    }
    ...
    fn reconcile_keeps_local_when_close_is_pending_despite_server_open() {
        ...
        pf.open_shift = Some(Some(Box::new(open_shift_model())));
        assert!(matches!(
            reconcile(&pf, TELLER_A, false, true),
            ShiftReconcile::KeepLocal
        ));
    }
    ...
    fn local_shifts_reconstructs_offline_opened_and_closed_shifts() {
        ...
        store
            .kv_put(
                ...
                r#"{"tellers":[{"user_id":"T1","name":"Sara"}]}"#,
            )
            .unwrap();
        ...
        req.opened_at = Some(Some(
            chrono::DateTime::parse_from_rfc3339("2026-06-20T09:00:00+00:00").unwrap(),
        ));
        let cmd = OpenShiftCommand {
            branch_id: "B1".into(),
            request: req,
        };
        store
        ...
        creq.closed_at = Some(Some(
            chrono::DateTime::parse_from_rfc3339("2026-06-20T18:00:00+00:00").unwrap(),
        ));
        let ccmd = CloseShiftCommand {
            shift_id: sid.to_string(),
            request: creq,
        };
        store
            .enqueue(&crate::store::NewOutboxOp {
    ...
    fn queued_close_overlay_marks_offline_closed_shifts() {
        ...
        req.closed_at = Some(Some(
            chrono::DateTime::parse_from_rfc3339("2026-06-20T18:00:00+00:00").unwrap(),
        ));
        let cmd = CloseShiftCommand {
            shift_id: "S1".into(),
            request: req,
        };
        store
        ...
        assert_eq!(
            *declared, 48_000,
            "carries the locally-declared closing cash"
        );
        assert!(
            closed_at
                .as_deref()
                .unwrap()
                .starts_with("2026-06-20T18:00:00"),
            "carries the close time"
        );
        assert!(
            !overlay.contains_key("S2"),
            "only shifts with a queued close are overlaid"
        );
    }
    ...
    fn shift_summary_view_is_open_only_for_exact_open_status() {
        let mk = |status: &str| models::Shift {
            status: status.into(),
            ..Default::default()
        };
    ...
    fn clear_makes_current_none() {
        ...
        assert_eq!(
            store.kv_get(CURRENT_SHIFT_KEY).unwrap().as_deref(),
            Some("null")
        );
    }
    ...
    fn reconcile_close_pending_irrelevant_when_server_has_no_shift() {
        ...
        let pf = models::ShiftPreFill::new(false, 0);
        assert!(matches!(
            reconcile(&pf, TELLER_A, true, true),
            ShiftReconcile::KeepLocal
        ));
        assert!(matches!(
            reconcile(&pf, TELLER_A, false, true),
