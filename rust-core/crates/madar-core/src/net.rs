...
impl ApiClient {
    ...
    fn observe_clock(&self, resp: &reqwest::Response) {
            ...
            .and_then(parse_http_date)
        {
            ...
            self.clock_skew
                .store(clamp_skew(skew), std::sync::atomic::Ordering::Relaxed);
        }
    }
    ...
    pub async fn ping(&self) -> CoreResult<Option<i64>> {
        ...
        let skew = resp
            ...
            .and_then(parse_http_date)
            .map(|server_epoch| {
                clamp_skew(server_epoch.saturating_sub(chrono::Utc::now().timestamp()))
            });
        Ok(skew)
    }
...
pub(crate) fn map_api_error<T>(e: ApiError<T>) -> CoreError {
    match e {
        ...
        ApiError::Serde(se) => CoreError::Internal {
            detail: format!("decode: {se}"),
        },
...
pub(crate) fn status_to_error(status: u16, body: &str) -> CoreError {
    ...
    // A punch refusal keeps the whole body: its `vars` (the distance, the
    // opening time…) are what the core words it with (`dawam::punch_words`).
...
mod tests {
    ...
    fn status_to_error_401_with_backend_envelope_is_unauthenticated() {
        ...
        assert!(matches!(
            status_to_error(
                401,
                r#"{"error":"Invalid or expired token","code":"unauthorized"}"#
            ),
    ...
    fn status_to_error_401_without_backend_envelope_is_offline() {
        ...
        assert!(matches!(
            status_to_error(401, "<html>Wi-Fi login</html>"),
            CoreError::Offline { .. }
        ));
        assert!(matches!(
            status_to_error(401, ""),
        ...
        assert!(!is_connectivity_failure(&status_to_error(
            401,
            r#"{"error":"wrong pin"}"#
        )));
    }
    ...
    fn clamp_skew_bounds_to_plus_minus_48h() {
        ...
        assert_eq!(
            clamp_skew(MAX_PLAUSIBLE_SKEW_SECS + 1),
            MAX_PLAUSIBLE_SKEW_SECS
        );
    }
    ...
    fn status_maps_to_variant() {
        ...
        assert!(matches!(
            status_to_error(401, body),
            CoreError::Unauthenticated { .. }
        ));
        assert!(matches!(
            status_to_error(403, body),
            CoreError::Forbidden { .. }
        ));
        assert!(matches!(
            status_to_error(400, body),
            CoreError::Validation { .. }
        ));
        assert!(matches!(
            status_to_error(409, body),
            CoreError::Server { status: 409, .. }
        ));
        assert!(matches!(
            status_to_error(503, body),
            CoreError::Transient { .. }
        ));
    }
    ...
    fn extracts_backend_error_message() {
        ...
        if let CoreError::Validation {
            detail: message, ..
        } = status_to_error(400, "not json")
        {
    ...
    fn extract_error_message_reads_message_and_detail_keys() {
        ...
        assert_eq!(
            extract_error_message(r#"{"message":"middleware says no"}"#).as_deref(),
            Some("middleware says no")
        );
        assert_eq!(
            extract_error_message(r#"{"detail":"422 detail"}"#).as_deref(),
            Some("422 detail")
        );
    ...
    fn status_to_error_422_is_validation() {
        assert!(matches!(
            status_to_error(422, r#"{"error":"bad field"}"#),
            CoreError::Validation { .. }
        ));
    }
    ...
    fn map_api_error_response_error_classifies_by_status() {
        ...
        assert!(matches!(
            map_api_error(ApiError::ResponseError(resp)),
            CoreError::Unauthenticated { .. }
        ));
    ...
    fn connectivity_failure_covers_portals_not_credential_rejections() {
        ...
        assert!(is_connectivity_failure(&CoreError::Offline {
            detail: "x".into()
        }));
        assert!(is_connectivity_failure(&CoreError::Transient {
            detail: "x".into()
        }));
        assert!(
            is_connectivity_failure(&CoreError::Internal {
                detail: "decode: expected value".into()
            }),
            "a captive-portal HTML body (decode error) must fall back to offline"
        );
        for status in [408u16, 407, 511] {
            assert!(
                is_connectivity_failure(&CoreError::Server {
                    status,
                    code: "x".into(),
                    detail: "x".into()
                }),
        ...
        assert!(!is_connectivity_failure(&CoreError::Unauthenticated {
            detail: "wrong pin".into()
        }));
        assert!(!is_connectivity_failure(&CoreError::Forbidden {
            resource: "api".into(),
            action: "disabled".into()
        }));
        assert!(!is_connectivity_failure(&CoreError::Validation {
            field: String::new(),
            detail: "bad".into()
        }));
        assert!(!is_connectivity_failure(&CoreError::Server {
            status: 404,
            code: "x".into(),
            detail: "x".into()
        }));
        // A non-decode Internal (a real local bug) must not masquerade as offline.
        assert!(!is_connectivity_failure(&CoreError::Internal {
    ...
    fn parse_http_date_known_imf_fixdate() {
        ...
        assert_eq!(
            parse_http_date("Sun, 21 Jun 2026 10:00:00 GMT"),
            Some(expected)
        );
    }
