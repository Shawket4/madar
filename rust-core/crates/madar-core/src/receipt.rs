...
pub struct ReceiptLabels {
    ...
    pub locale: String,
    /// The zone every printed time is shown in (the branch's effective zone).
    pub tz: chrono_tz::Tz,
}
...
pub fn layout(receipt: &ReceiptView, ctx: &EscPosCtx) -> Vec<Line> {
    ...
    let bold_left = |text: String| Line {
        text,
        align: Align::Left,
        bold: true,
        size: Size::Normal,
    };
    let bold_center = |text: String| Line {
        text,
        align: Align::Center,
        bold: true,
        size: Size::Normal,
    ...
    let dt = crate::timefmt::format_in(
        lab.tz,
        &receipt.created_at,
    ...
    if receipt.delivery_fee_minor > 0 {
        out.push(Line::plain(row(
            &lab.delivery_fee,
            &money(receipt.delivery_fee_minor, cur),
            w,
        )));
    }
    out.push(bold_left(row(
...
pub fn layout_till_report(
    ...
    out.push(Line::plain(row(
        &labels.opening,
        &money(report.opening_cash_minor, cur),
        w,
    )));
    out.push(Line::plain(row(
        &labels.payments,
        &money(report.net_payments_minor, cur),
        w,
    )));
    if report.cash_in_minor > 0 {
        out.push(Line::plain(row(
            &labels.cash_in,
            &money(report.cash_in_minor, cur),
            w,
        )));
    }
    if report.cash_out_minor > 0 {
        out.push(Line::plain(row(
            &labels.cash_out,
            &money(-report.cash_out_minor, cur),
            w,
        )));
    }
    out.push(Line {
    ...
    if !report.cash_movements.is_empty() {
        ...
        for m in &report.cash_movements {
            let label = if m.note.trim().is_empty() {
                m.moved_by_name.clone()
            } else {
                m.note.clone()
            };
            out.push(Line::plain(row(
                &format!("  {label}"),
                &money(m.amount_minor, cur),
                w,
            )));
        }
    }
...
mod tests {
    ...
    fn till_report_layout_has_drawer_lines_and_methods() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text.starts_with("Cash in") && l.text.ends_with("30.00 EGP")));
        assert!(lines
            .iter()
    ...
    fn layout_cash_receipt_matches_flutter_structure() {
        ...
        assert!(text
            .iter()
            .any(|t| t.starts_with("2x Latte") && t.ends_with("90.00 EGP")));
        assert!(text
            .iter()
            .any(|t| t.starts_with("1x Croissant") && t.ends_with("35.00 EGP")));
        ...
        assert!(text
            .iter()
            .any(|t| t.starts_with("Payment") && t.ends_with("Cash")));
        ...
        assert!(
            lines
                .iter()
                .find(|l| l.text.contains("20/06/2026"))
                .unwrap()
                .bold
        );
        assert!(
    ...
    fn layout_shows_subtotal_discount_and_offline_hint_when_present() {
        ...
        let discount = lines
            .iter()
            .find(|l| l.text.starts_with("Discount"))
            .expect("discount row");
    ...
    fn layout_voided_stamp_and_teller_footer() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text.starts_with("Teller") && l.text.ends_with("Mona")));
    }
    ...
    fn layout_delivery_with_modifiers_and_bundle() {
        ...
        latte.addons = vec![ReceiptModifierView {
            name: "Oat milk".into(),
            price_minor: 800,
        }];
        latte.optionals = vec![ReceiptModifierView {
            name: "No sugar".into(),
            price_minor: 0,
        }];
        ...
        assert!(text
            .iter()
            .any(|t| t.starts_with("Customer") && t.ends_with("Sara")));
        assert!(text
            .iter()
            .any(|t| t.starts_with("Address:") && t.contains("Nile St")));
        ...
        assert!(text
            .iter()
            .any(|t| t.starts_with("Delivery Fee") && t.ends_with("20.00 EGP")));
    ...
    fn escpos_equals_encode_of_layout() {
        ...
        assert_eq!(
            escpos(&r, &c, PrinterBrand::Star),
            encode_star(&layout(&r, &c))
        );
    }
    ...
    fn encode_for_dispatches_by_brand() {
        ...
        assert_ne!(
            encode_for(PrinterBrand::Epson, &l),
            encode_for(PrinterBrand::Star, &l)
        );
    ...
    fn layout_subtotal_appears_for_delivery_fee_alone() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text.starts_with("Delivery Fee") && l.text.ends_with("15.00 EGP")));
    ...
    fn layout_delivery_in_mall_channel_flag() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text == "*** DELIVERY — In-Mall ***" && l.bold));
    }
    ...
    fn layout_delivery_optional_block_fields_are_each_conditional() {
        ...
        assert!(text
            .iter()
            .any(|t| t.starts_with("Zone") && t.ends_with("Zone 3")));
        assert!(text
            .iter()
            .any(|t| t.starts_with("Delivery Ref") && t.ends_with("DLV-9")));
        assert!(text
            .iter()
            .any(|t| t.starts_with("Payment (hint)") && t.ends_with("COD")));
        // notes prints as a free-form "{label} {value}" line, not a right-aligned row.
        assert!(text
            .iter()
            .any(|t| t.starts_with("Notes:") && t.contains("ring bell")));
    ...
    fn layout_non_delivery_prints_customer_in_footer() {
        ...
        assert_eq!(
            lines
                .iter()
                .filter(|l| l.text.starts_with("Customer"))
                .count(),
            1
        );
        assert!(lines
    ...
    fn layout_item_size_label_inlined_in_name() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text.starts_with("3x Tea (Large)") && l.text.ends_with("45.00 EGP")));
    }
    ...
    fn layout_order_ref_row_present_only_when_set() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text.starts_with("Ref:") && l.text.contains("POS-77")));
    ...
    fn layout_till_report_minimal_has_core_rows_no_optional_sections() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.bold && l.text.starts_with("Expected")));
    ...
    fn layout_till_report_blank_note_movement_labels_by_name() {
        ...
        assert!(lines
            .iter()
            .any(|l| l.text.trim_start().starts_with("Mona") && l.text.ends_with("5.00 EGP")));
