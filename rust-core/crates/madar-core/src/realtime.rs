    ...
    fn on_event(&self, event: RealtimeEvent) {
        ...
        if let Some(alert) = alert_for(&event.event_type, &event.data, &locale) {
            let fresh = self
                .dedup
                .lock()
                .map(|mut d| d.insert(&alert.tag))
                .unwrap_or(true);
            if fresh {
                self.player.play_ping();
                self.player
                    .post_notification(alert.title, alert.body, alert.tag);
                self.player.haptic();
            }
        }
    }
...
impl RealtimeClient {
    ...
    async fn open(
        ...
        let resp = rb
            .send()
            .await
            .map_err(|e| crate::net::classify_reqwest(&e))?;
...
async fn run_supervisor(
    ...
    loop {
        match client
            .open(&branch_id, &topics_csv, last_event_id.as_deref())
            .await
        {
            Ok(resp) => {
                attempt = 0;
...
mod tests {
    ...
    fn handles_multibyte_utf8_split_across_chunks() {
        ...
        assert_eq!(
            out[0].data, "نقدي",
            "Arabic survives a mid-character chunk split"
        );
    }
    ...
    fn dedup_alerts_once_per_tag() {
        ...
        assert!(
            !d.insert("delivery.created:o1"),
            "repeat (LAN+cloud) → suppressed"
        );
        assert!(
