...
pub(crate) struct TillFigures {
    ...
    /// Cash spot checks on this till, oldest first (`report_figures`' `spot_checks`).
    pub spot_checks: Vec<super::spot::SpotRow>,
...
pub(crate) fn compute(conn: &Connection, till_id: &str, methods: &[Method]) -> CoreResult<Option<TillFigures>> {
    ...
    let unsynced: i64 = conn.query_row(
        "SELECT (SELECT COUNT(*) FROM outbox WHERE till_id=?1 AND status IN ('pending','inflight','dead')
                   AND op_type IN ('create_order','void_order','refund_order','cash_movement','cash_spot_check','settle_open_ticket','close_till','close_shift'))",
        params![till_id],
        |r| r.get(0),
    )?;
    ...
    Ok(Some(TillFigures {
        ...
        payment_summary,
        close_methods,
        spot_checks: super::spot::for_till(conn, till_id)?,
