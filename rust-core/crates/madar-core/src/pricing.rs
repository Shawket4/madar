...
mod tests {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // MENU-UNIFICATION MIGRATION PARITY GUARDRAIL
    //
    // The backend migrated modifiers from flat "addons + slots" to grouped
    // "modifier groups + options" (stable ids: modifier_option.id == old
    // addon_item.id). The server resolves each selected option's CHARGED delta
    // (`price_modifier`) via the same effective-price rules (validated
    // byte-identical reading the shim views), so a given selection charges the
    // SAME delta whether presented the legacy or the new way.
    //
    // `price_cart` sums those deltas, so it is invariant under modifier
    // regrouping/reordering — i.e. the migration is PRICE-NEUTRAL for the POS.
    // These vectors lock that in.
    // ─────────────────────────────────────────────────────────────────────
    #[test]
    fn migration_price_parity_modifier_model_is_representation_invariant() {
        // A latte (5000) + oat-milk swap (+1000 ×1) + extra shot (+500 ×2) +
        // vanilla-syrup optional (300). Legacy: flat `addons`. New: the same
        // options grouped (milk group / extras group) and in a different order.
        let legacy = CartLine {
            quantity: 1,
            unit_price: 5000,
            is_bundle: false,
        ...
        assert_eq!(
            a, b,
            "the modifier-model migration must not change what a customer pays"
        );
        // Golden: subtotal 5000+1000+1000+300 = 7300; disc 10% = 730; taxable 6570;
        // tax round(6570*0.14)=round(919.8)=920; total 7490.
        assert_eq!(b.subtotal_minor, 7300);
        assert_eq!(b.discount_minor, 730);
        assert_eq!(b.taxable_minor, 6570);
        assert_eq!(b.tax_minor, 920);
        assert_eq!(b.total_minor, 7490);
    }
...
mod proptests {
    ...
    fn arb_addon() -> impl Strategy<Value = AddonSel> {
        (0i64..100_000, 0i64..20).prop_map(|(price_modifier, quantity)| AddonSel {
            price_modifier,
            quantity,
        })
    }
    ...
    /// `loyalty_reward_vectors.json` is generated by madar-shared
    /// (`madar_money::loyalty::vectors`) from the rule the server runs.
    #[test]
    fn reward_pricing_agrees_with_the_servers_vectors() {
        ...
        let raw = madar_money::vectors::LOYALTY_REWARD;
